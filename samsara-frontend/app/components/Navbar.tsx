...
const ExperiencesOverlay = dynamic(() => import("./ExperiencesOverlay"), { ssr: false });
    ...
    <>
      <style>{`
        ...
        .global-nav-exp-btn {
          ...
          color: #ffffff; font-size: 0.75rem; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase;
          transition: color 0.3s; position: relative; font-family: 'Inter', sans-serif;
        }
        .global-nav-exp-btn::after {
          content: ''; position: absolute; bottom: -4px; left: 0;
          width: 0; height: 1px; background: #C9A84C; transition: width 0.3s;
        }
        .global-nav-exp-btn:hover { color: #E2C97E; }
        .global-nav-exp-btn:hover::after { width: 100%; }
        .global-mobile-exp-btn {
          background: none; border: none; cursor: pointer;
          font-family: 'TT Fors', sans-serif; font-weight: 300;
          font-size: clamp(2rem, 8vw, 3rem);
          color: #ffffff; letter-spacing: 0.04em; transition: color 0.3s; padding: 0;
        }
        .global-mobile-exp-btn:hover { color: #E2C97E; }
      ...
      <nav className={`global-navbar${scrolled ? " scrolled" : ""}`}>
        ...
        <ul className="global-nav-links">
          ...
          <li><button className="global-nav-exp-btn" onClick={() => setExpOpen(true)}>Experiences</button></li>
          <li><a href="/feeling-engine">Feelings Engine</a></li>
          <li><a href="/#about">About</a></li>
        </ul>
        <a href="/feeling-engine" style={{ textDecoration: "none" }}>
          <button className="global-nav-cta">Plan My Trip</button>
        </a>
        <button
      ...
      {/* EXPERIENCES OVERLAY */}
      {expOpen && <ExperiencesOverlay onClose={() => setExpOpen(false)} />}
      ...
      <div className={`global-mobile-menu${menuOpen ? " open" : ""}`}>
        ...
        <button className="global-mobile-exp-btn" onClick={() => { closeMenu(); setExpOpen(true); }}>Experiences</button>
        <div className="global-mobile-divider" />
        <a href="/feeling-engine" onClick={closeMenu}>Feelings Engine</a>
        <div className="global-mobile-divider" />
        <a href="/#about" onClick={closeMenu}>About</a>
        <a href="/feeling-engine" onClick={closeMenu} style={{ textDecoration: "none" }}>
          <button className="global-mobile-cta">Plan My Trip</button>
        </a>
      </div>
    </>
  );
